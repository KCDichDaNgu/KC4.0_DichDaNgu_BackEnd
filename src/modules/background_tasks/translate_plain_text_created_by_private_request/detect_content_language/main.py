...
logger = Logger('Task: translate_plain_text_in_private_request.detect_content_language')
...
async def main():
    ...
    try:
        ...
        if len(tasks_id) == 0: 
            logger.debug(
                msg=f'An task translate_plain_text_in_private_request.detect_content_language end in {datetime.now()}\n'
            )

            print(f'An task translate_plain_text_in_private_request.detect_content_language end in {datetime.now()}\n')
            return
