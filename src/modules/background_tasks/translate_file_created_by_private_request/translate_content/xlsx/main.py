...
async def main():
    ...
    logger.debug(
        msg=f'New task translate_file_created_by_private_request.translate_content.xlsx run in {datetime.now()}'
    )

    print(f'New task translate_file_created_by_private_request.translate_content.xlsx run in {datetime.now()}')
    ...
    try:
        ...
        if len(tasks_id) == 0: 
            logger.debug(
                msg=f'An task translate_file_created_by_private_request.translate_content.xlsx end in {datetime.now()}\n'
            )
            print(f'An task translate_file_created_by_private_request.translate_content.xlsx end in {datetime.now()}\n')
            return
