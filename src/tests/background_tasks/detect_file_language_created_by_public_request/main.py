...
from modules.background_tasks.detect_file_language_created_by_public_request.main import read_task_result, mark_invalid_tasks, execute_in_batch
...
df = pandas.read_csv('src/tests/background_tasks/detect_file_language_created_by_public_request/sample_data/detect_file_language_data.csv')
...
async def get_input_data():
    ...
    try:
        ...
        if len(tasks_id) == 0: 
            print('Invalid task detect_file_language_created_by_public_request')
            return
...
async def test_read_task_result():
    print ('----------------Detect file language created by public request: Test Read Task Result----------------')
...
async def test_mark_invalid_tasks():
    print ('----------------Detect file language created by public request: Test Mark Invalid Tasks----------------')
...
async def test_execute_in_batch(allowed_concurrent_request):
    print ('----------------Detect file language created by public request: Test Execute In Batch----------------')
